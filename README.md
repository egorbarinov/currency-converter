...
Стек технологий, на котором реализовано приложение: SpringBoot, SpringSecurity, PostgreSQL, JPA, шаблонизатор Thymeleaf, Document Object Model.
...
[http://www.cbr.ru/scripts/XML_daily.asp?date_req=30/10/2020](http://www.cbr.ru/scripts/XML_daily.asp?date_req=30/10/2020) 
Значения из полученных по ссылке XML-данных сохраняются в объекте типа Document, где они преобразовываются в сущности и передаются в базу данных Postgres. 
При старте программы значения курсов автоматически обновляются.
...
2) Обработка входящего url с xml-данными по валютам, "парсинг" данных и их проекция на странице "index".
3) Конвертер валюты.
