...
6) Валидация регистрационной формы: требуется заполнить все формы для регистрации пользователя.
7) Привел в порядок методы по работе с фронтом. Теперь в логике работы используются dto вместо сущностей dao.
8) Настроено автоматическое обновление курсов валют, по указанному интервалу времени.
...
3) Проработать логику вывода данных по валютам, в зависимости от дней недели
4) Настроить регистрацию и активацию пользователя по отправке валидационного ключа на почту.
...
6) Изменена настройка spring.jpa.hibernate.ddl-auto = validate. Данная настройка сверяет данные, посылаемые в DB,
 и в случае их отсутствия добававляет новые записи в таблицы.
